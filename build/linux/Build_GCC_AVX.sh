...
# features enabled, mirroring build/MSYS_jpsdr/GCC_AVX.sh.
#
...
mkdir -p 8bit_gcc_avx 10bit_gcc_avx 12bit_gcc_avx

cd 12bit_gcc_avx
cmake ../../../source -DCMAKE_BUILD_TYPE=Release -DENABLE_LIBVMAF=ON -DENABLE_SCC_EXT=ON -DENABLE_MULTIVIEW=ON -DENABLE_ALPHA=ON -DENABLE_VAPOURSYNTH=ON -DENABLE_AVISYNTH=ON -DHIGH_BIT_DEPTH=ON -DENABLE_HDR10_PLUS=ON -DEXPORT_C_API=OFF -DENABLE_SHARED=OFF -DENABLE_CLI=OFF -DMAIN12=ON -DENABLE_LTO=ON -DCMAKE_CXX_FLAGS="-mavx" -DCMAKE_C_FLAGS="-mavx"
make ${MAKEFLAGS}
cp libx265.a ../8bit_gcc_avx/libx265_main12.a

cd ../10bit_gcc_avx
cmake ../../../source -DCMAKE_BUILD_TYPE=Release -DENABLE_LIBVMAF=ON -DENABLE_SCC_EXT=ON -DENABLE_MULTIVIEW=ON -DENABLE_ALPHA=ON -DENABLE_VAPOURSYNTH=ON -DENABLE_AVISYNTH=ON -DHIGH_BIT_DEPTH=ON -DENABLE_HDR10_PLUS=ON -DEXPORT_C_API=OFF -DENABLE_SHARED=OFF -DENABLE_CLI=OFF -DMAIN12=OFF -DENABLE_LTO=ON -DCMAKE_CXX_FLAGS="-mavx" -DCMAKE_C_FLAGS="-mavx"
make ${MAKEFLAGS}
cp libx265.a ../8bit_gcc_avx/libx265_main10.a

cd ../8bit_gcc_avx
cmake ../../../source -DCMAKE_BUILD_TYPE=Release -DENABLE_LIBVMAF=ON -DENABLE_SCC_EXT=ON -DENABLE_MULTIVIEW=ON -DENABLE_ALPHA=ON -DENABLE_VAPOURSYNTH=ON -DENABLE_AVISYNTH=ON -DHIGH_BIT_DEPTH=OFF -DENABLE_HDR10_PLUS=OFF -DEXPORT_C_API=ON -DENABLE_SHARED=OFF -DENABLE_CLI=ON -DMAIN12=OFF -DENABLE_LTO=ON -DLINKED_10BIT=ON -DLINKED_12BIT=ON -DEXTRA_LIB="x265_main10.a;x265_main12.a" -DEXTRA_LINK_FLAGS=-L. -DCMAKE_CXX_FLAGS="-mavx" -DCMAKE_C_FLAGS="-mavx"
make ${MAKEFLAGS}
